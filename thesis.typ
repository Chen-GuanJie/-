...
#abstract(keywords: (
  ...
  本文在80个移动应用程序和160个设计稿上进行了大量实验，结果表明：（1）GUIPilot在屏幕不一致性检测方面可达到99.8\%的精确率和98.6\%的召回率，分别比当前最先进的方法GVT高出66.2\%和56.6\%；（2）GUIPilot在流程不一致性检测方面实现了零错误率。此外，本文在某交易类移动应用上开展的工业案例研究表明，GUIPilot检测出了9个应用缺陷，且所有缺陷均得到了原应用专家的确认。本文代码已开源，地址为：https://github.com/code-philia/GUIPilot。
]
