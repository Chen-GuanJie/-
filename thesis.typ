...
#abstract(keywords: (
  ...
  本研究提出了#(tool)方法，用于检测移动端设计稿与其实现之间的不一致性。移动端设计通常由设计稿组成，这些设计稿规定了：（1）预期的屏幕外观；（2）预期的屏幕行为，即屏幕之间的跳转方式。给定设计稿及其对应的应用实现，#(tool)能够同时报告屏幕不一致性和流程不一致性。
  在屏幕不一致性检测方面，#(tool)将每个屏幕抽象为控件容器，其中每个控件由其位置、宽度、高度和类型表示。通过定义控件的偏序关系以及屏幕中控件替换、插入和删除的代价，本文将屏幕匹配问题转化为可估计的控件对齐问题。在流程不一致性检测方面，本文将规定的GUI跳转转换为移动屏幕上的逐步操作。为此，本文提出了一种面向视觉-语言模型的视觉提示方法，以推断屏幕上针对特定控件的操作。通过这种方式，可以验证实现中预期跳转的存在与否。
  本文在80个移动应用程序和160个设计稿上进行了大量实验，结果表明：（1）#(tool)在屏幕不一致性检测方面可达到99.8\%的精确率和98.6\%的召回率；（2）#(tool)在流程不一致性检测方面实现了零错误率。此外，本文在某交易类移动应用上开展的工业案例研究表明，#(tool)检测出了9个应用缺陷，且所有缺陷均得到了原应用专家的确认。
]
