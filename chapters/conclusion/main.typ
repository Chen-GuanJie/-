...
针对静态界面的视觉一致性，本文提出了一种无需依赖代码侵入的检测方法。该方法结合了基于深度学习的目标检测技术与基于递归布局分解的组件对齐算法。通过引入加权最长公共子序列（LCS）匹配机制，该方法有效解决了传统像素级比对或坐标强匹配方法在面对不同分辨率、屏幕适配及布局微调时的鲁棒性不足问题，实现了从“像素”到“语义组件”的细粒度比对，能够精准识别控件的缺失、多余及属性（文本、颜色、类型）变更。
