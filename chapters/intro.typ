...
本文提出了 #(tool)，一种面向移动应用设计与其实现一致性检测的新方法，具备同时报告屏幕级和流程级（即跳转）不一致性的能力。
...
针对流程不一致性检测，该方法利用先进的视觉-语言模型（VLM），将设计稿中描述的 GUI 跳转逻辑转换为移动设备上的具体操作步骤（如点击、长按、文本输入等）。为抑制 VLM 可能产生的幻觉问题，本文引入了*视觉提示（Visual Prompt）*技术，引导模型仅在限定的相关控件集合中推断动作。借助该技术，#(tool) 能够依据设计稿自动导航至后续屏幕，并验证实际跳转是否符合预期或存在遗漏。

本文基于 80 个移动应用程序（涵盖 4 种应用类型和 160 个设计稿）进行了系统评估。实验结果表明：（1）#(tool) 在屏幕不一致性检测上实现了 94.5% 的精确率和 99.6% 的召回率，显著优于 GVT 等现有最先进方法（分别提升了 66.2% 和 56.6%）；（2）在流程不一致性检测方面，#(tool) 未出现错误报告；（3）方法执行效率高，屏幕匹配算法平均耗时仅为 0.001 秒，单次跳转平均耗时 0.19 秒。此外，通过在一款拥有 3200 万用户的工业级交易应用上开展的案例研究，#(tool) 成功识别出 9 个实际的设计违规缺陷，所有缺陷均获得了领域专家的确认。
