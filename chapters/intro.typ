...
@chp:background，设计稿一致性验证的相关理论。本章系统回顾了与本文工作密切相关的支撑技术。首先介绍了移动端设计稿的规范定义及常见不一致性类型，为后续的检测工作奠定基础；其次，分析了现有的移动GUI自动化测试技术及其在设计验证方面的不足；随后，探讨了计算机视觉与大语言模型在GUI理解与任务自动化领域的最新进展，为本文利用VLM进行动作生成提供了理论依据；最后，对包括GVT在内的相关一致性检测工具进行了综述与分析。//todo
