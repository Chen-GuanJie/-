...
本文在 80 个移动应用程序上进行了评估，涵盖 4 种应用类型和 160 个设计原型。结果表明：（1）#(tool) 在检测屏幕不一致性方面达到了 94.5% 的精确率和 99.6% 的召回率，分别优于 GVT 等最先进方法 66.2% 和 56.6%；（2）#(tool) 在检测流程不一致性方面报告零错误；（3）#(tool) 效率高，屏幕匹配算法平均耗时 0.001 秒，跳转平均耗时 0.19 秒。此外，我们与工业界合作者在一个拥有 3200 万用户的交易应用程序上进行了案例研究，#(tool) 检测到了 9 个应用不一致缺陷，所有缺陷均得到了应用专家的确认。

== 本文主要贡献

本文的主要贡献总结如下：

- 我们提出了 #(tool)，这是一种系统地检测设计原型与移动应用实现之间不一致性的解决方案。据我们所知，#(tool) 是首个专为工业界广泛采用的设计原型量身定制的端到端 GUI 测试解决方案。
