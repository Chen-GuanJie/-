...
- *语义变化——控件交换（Swapped widgets）：* 随机选取 5% 的控件对，并将其类型替换为视觉或功能上不同的其他控件（例如将按钮替换为文本框），以此引入语义层面的不一致。
...
本研究选取了 GVT @moran2018automated 作为主要的对比基准（Baseline）。GVT 是目前公认的最先进的移动应用界面自动化比对工具之一，本实验严格复现了其配置参数以确保比较的公平性。此外，为探究大模型直接应用于此类任务的可行性及现有局限，实验还设置了一个基于视觉-语言模型（VLM）的简单基准（Naive Baseline）。该方法将标注有控件 ID 的前后两帧屏幕截图直接输入 VLM，并通过自然语言提示（Prompting）引导模型识别并输出缺失、新增或被修改的控件列表。
...
*对语义交换的识别能力：*
#imagex(
  subimagex(image("../../figures/gvt_fails_guipilot_success/1_gvt.png"), caption: [GVT (示例 1)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/1_guipilot.png"), caption: [#(tool) (示例 1)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/2_gvt.png"), caption: [GVT (示例 2)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/2_guipilot.png"), caption: [#(tool) (示例 2)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/5_gvt.png"), caption: [GVT (示例 3)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/5_guipilot.png"), caption: [#(tool) (示例 3)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/6_gvt.png"), caption: [GVT (示例 4)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/6_guipilot.png"), caption: [#(tool) (示例 4)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/9_gvt.png"), caption: [GVT (示例 5)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/9_guipilot.png"), caption: [#(tool) (示例 5)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/11_gvt.png"), caption: [GVT (示例 6)]),
  subimagex(image("../../figures/gvt_fails_guipilot_success/11_guipilot.png"), caption: [#(tool) (示例 6)]),
...
#imagex(
  subimagex(
    image("../../figures/guipilot_fp/2.png", width: 82%),
    caption: [FP 示例 1],
  ),
  subimagex(
    image("../../figures/guipilot_fp/5.png", width: 82%),
    caption: [FP 示例 2],
  ),
  subimagex(
    image("../../figures/guipilot_fp/9.png", width: 82%),
    caption: [FP 示例 3],
  ),
  subimagex(
    image("../../figures/guipilot_fn/3.png", width: 82%),
    caption: [FN 示例 1],
  ),
  columns: (1fr, 1fr),
