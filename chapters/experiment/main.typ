...
+ *RQ3: 核心控件性能评估（Component-level Evaluation）。* 构成工具的关键模块是否具备足够的准确性以支撑整体任务？具体包括：
...
在“模拟设计稿”的生成过程中，邀请了四位具有至少两年专业软件开发与测试经验的专家参与标注。针对每一个选定的应用，专家需依据其核心功能定义两个典型的用户操作场景（例如“关注特定的社交账号”或“修改应用通知偏好设置”）。专家详细记录了完成这些流程所需的所有屏幕截图、界面层级结构以及屏幕间的转换动作链。这些经过人工校验的流程数据被用作实验中的“标准设计稿”（Ground Truth）。最终，实验数据集共包含从 80 个应用中提取的 160 套设计稿流程，每套流程平均包含约 8 个屏幕节点。
