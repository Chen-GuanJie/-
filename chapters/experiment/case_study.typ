...
#(tool) 采用的基于序理论的匹配算法在此类场景下展现了卓越的鲁棒性。该算法首先通过递归投影切割将二维空间中的控件布局转化为具有拓扑逻辑的一维偏序序列，从而剥离了对绝对坐标的依赖。随后，通过将匹配问题建模为加权最长公共子序列求解，算法能够在容忍非线性间隙与局部结构差异的前提下，从完整的设计稿序列中精准定位并对齐实现端可见的局部子序列。如图所示，即使在设计稿与实现界面的纵横比、分辨率乃至布局密度存在巨大差异的情况下，#(tool) 依然准确识别出了多余或缺失的控件，并建立了正确的控件级映射关系。
...
*设计规范的完整性缺失*
通过对工业级设计稿的深入分析，研究发现现有的交付物规范仍存在不足。许多设计稿缺乏关键的元数据，特别是从应用入口（如主屏幕）导航至特定设计场景的“上下文路径”往往缺失。这种路径信息的断层不仅阻碍了自动化测试工具的接入（难以确定测试的起始状态），也增加了新加入开发者的理解成本。本研究在实验中不得不通过人工手段补全这些导航信息，但这提示未来的设计交付标准应显式包含状态机或导航图信息。
