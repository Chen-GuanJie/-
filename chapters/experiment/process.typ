...
(i) *资源引用错误*：代码中错误的资源 ID 导致事件侦听器绑定到了错误的 UI 组件上；
(ii) *状态同步缺失*：应用的全局状态（如登录态、数据提交状态）未及时更新，导致逻辑分支错误；
(iii) *事件拦截冲突*：父容器或相邻层级的控件错误地拦截了触摸事件，使得目标控件无法响应。

基于上述理论分析，本实验设计了一套自动化变异策略以模拟真实的流程缺陷。针对测试集中的每一条流程，随机选择一个状态转移步骤 $chevron.l bf(s)_t a r, d e s c, bf(s)_t a r chevron.r$ 进行变异注入。具体的变异模式设计如下：

+ *目标状态变异（Target Mutation，模拟状态回退）：* 篡改应用执行逻辑，使其在应当跳转至新界面 $bf(s)_t a r$ 时，错误地返回或停留在前一界面 $bf(s)_s r c$。这种情形通常模拟了因网络请求通过失败或本地数据未同步而导致的状态更新失败（例如，用户提交表单后本应进入“成功页”，却因提交失败逻辑被错误引导回“填写页”）。
...
+ *检测的准确性：* 无论是状态回退导致的界面停滞，还是交互错位引发的非预期跳转（如跳入错误的子页面），其产生的实际界面结果与设计稿预期的目标界面 $bf(s)_t a r$ 在视觉结构与内容上均存在显著差异。实验选定的相似度阈值成功区分了这些差异与正常的渲染微变，验证了所选阈值的有效性。
