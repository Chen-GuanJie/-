...
实验环境部署于 Linux 平台，并在该环境部署了一个轻量且可控的 Android 运行虚拟环境。所有的交互操作均通过 UiAutomator2 @uiautomator2 框架与 Android Debug Bridge (ADB) 接口自动化驱动。实验遵循 @img:process-inconsistency 所示的标准工作流，重点考察系统是否能在变异发生点准确识别出屏幕跳转异常并及时中止流程。
...
+ *检测的准确性：* 无论是状态回退导致的界面停滞，还是交互错位引发的非预期跳转（如跳入错误的子页面），其产生的实际界面结果与设计稿预期的目标界面 $screen_tar$ 在视觉结构与内容上均存在显著差异。实验选定的相似度阈值成功区分了这些差异与正常的渲染微变，验证了所选阈值的有效性。
