...
上述方法通常针对通用的设计准则进行检测，包括Apple的人机界面指南@apple2024accessibility 和Google的无障碍设计规范@google2024accessibility。然而，在实际应用开发中，设计师通常会基于产品需求创建特定的设计稿（Mock-up），要求最终实现的GUI与设计稿高度一致。针对这一需求，GVT@zhang2024learning 提出了一套专门用于检测设计稿与实现不一致问题的方法。该方法建立了详细的不一致问题分类体系，并设计了精确的控件匹配算法，能够在像素级别比较设计稿与实际GUI之间的差异，包括颜色偏差、边距不一致、字体大小错误等细粒度问题。
...
近年来，基于深度学习的方法已成为GUI控件检测的主流技术@cao2024comprehensive @hong2024cogagent @wu2024osatlas。这些方法利用卷积神经网络（CNN）或视觉Transformer等架构，能够自动学习GUI控件的视觉特征，在控件定位精度和泛化能力方面均显著优于传统方法。特别是随着视觉语言模型的发展，如CogAgent@hong2024cogagent 和OS-ATLAS@wu2024osatlas，GUI控件检测能力得到了进一步提升。这些模型不仅能够准确识别控件的位置和类型，还能理解控件的功能语义，为后续的测试操作提供更丰富的上下文信息。
=== GUI控件匹配技术
