...
视觉语言模型（Vision-Language Model, VLM）是连接计算机视觉与自然语言处理的关键技术，旨在赋予模型同时理解和处理视觉（图像、视频）与文本信息的能力。早期的VLM主要聚焦于图像描述任务（Image Captioning），旨在根据输入图像生成自然语言描述。2015年，Vinyals等人提出的"Show and Tell"模型首次将卷积神经网络（CNN）与循环神经网络（RNN）结合，实现了端到端的图像描述生成，为视觉与语言的结合奠定了重要基础 @vinyals2015show。随后，研究逐步扩展到更复杂的视觉问答（Visual Question Answering, VQA）领域，Antol等人提出了首个大规模VQA数据集，要求模型基于图像与自然语言问题进行跨模态推理 @antol2015vqa。随着注意力机制（Attention Mechanism）的引入，Xu等人提出的软注意力模型 @xu2015show 与Anderson等人提出的自底向上与自顶向下注意力机制 @anderson2018bottom 显著提升了模型在细粒度跨模态对齐上的能力。

进入预训练时代，受BERT等语言模型启发，LXMERT @tan2019lxmert 和ViLBERT @lu2019vilbert 等工作通过大规模图文对的多任务预训练学习通用的跨模态表示，推动了VLM从专用模型向通用基础模型的转变。2021年，OpenAI发布的CLIP模型 @radford2021learning 利用对比学习在4亿图文对上进行了大规模弱监督训练，展现了强大的零样本迁移能力。在此基础上，BLIP @li2022blip 实现了理解与生成任务的统一，而BLIP-2 @li2023blip2 通过轻量化适配器将图像编码器与大语言模型高效衔接，大幅提升了跨模态建模效率。DeepMind提出的Flamingo模型 @alayrac2022flamingo 展示了多模态少样本学习的潜力，仅依赖少量上下文提示即可在多种任务中取得优异表现。

2023年以后，随着多模态大模型的兴起，VLM的研究重心转向构建具备更强通用能力的模型。GPT-4 @openai2023gpt4 首次具备了处理文本与图像混合输入的能力，不仅在传统语言任务上表现出色，还在图像理解与复杂推理中展现了接近人类的水平。同时期推出的Gemini系列 @team2023gemini 和Claude系列 @anthropic2024claude 进一步整合了视频、音频等多模态信息，实现了更广泛的任务覆盖与更自然的交互体验。这些多模态大模型的出现，推动了VLM从单一任务工具转向通用人工智能应用基座，并在智能体研究中迅速落地，涌现出UFO、Claude Computer Use等基于视觉理解的GUI智能体。
