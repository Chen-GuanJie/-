...
== 目标检测
我们采用基于视觉的目标检测流水线来识别设计稿和实现界面中的 UI 控件。下文详细描述该子系统的任务定义、数据与标注、模型选择与训练、推理与后处理、评价指标、常见失败模式及缓解措施，便于非计算机背景读者理解。
...
数据与标注
- 数据来源：真实应用截图与设计稿图。为保证泛化能力，数据集应覆盖不同分辨率、主题（深色/浅色）、语言和布局风格。
