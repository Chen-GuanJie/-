...
== 结论
$"设计稿缺乏同步"$
在实际的 App 开发过程中，设计稿相对于当前的实现可能会过时。
这是因为设计稿的维护并不像移动应用程序那样严格。
在软件开发中，代码经历了持续集成和持续部署（CI/CD），确保其定期进行测试、更新和完善。
同样，设计稿也需要一种类似于 CI/CD 的系统化维护方法，以确保它们与应用程序并行演进，保持最新状态。
此外，当开发人员对应用程序实施变更时，应将这些修改传达给设计团队，以确保设计稿得到相应更新。
这需要进行战略性的调整，以提高设计稿在开发周期中的有效性和可用性。

$"设计稿规范建议"$
我们对工业级设计稿的案例研究表明，当前的设计稿设计方法仍有巨大的改进空间。
在工业级设计稿中，缺失了一些重要的规范细节。
在研究中，从应用程序的主屏幕导航到设计稿中指定的起始屏幕并非易事，
这阻碍了我们要应用本工具。
在研究中，我们手动将此类信息包含在工业设计稿中。
