...
*流程（Process）。* 流程 $bf(p)$ 被建模为一个有向连通图 $bf(p): G = ( bf(S), bf(E) )$。其中，$bf(S)$ 代表一组屏幕节点集合，$bf(E)$ 代表一组操作链集合。图中的每一条边由操作链 $bf(A) in bf(E)$ 构成，该操作链驱动应用从当前屏幕状态转换到下一个屏幕状态（或保持状态不变）。例如，@img:mockup-example 展示的登录流程由 5 个顶点（屏幕）组成，它们之间通过 4 条有向边（操作链或状态转移）相连。

*屏幕不一致性（Screen Inconsistency）。* 本文关注三种类型的屏幕级不一致：多余控件、缺失控件和语义变化。形式化地，记设计稿中特定屏幕的控件集合为 $bf(W)^tar$，实现端对应屏幕的控件集合为 $bf(W)$。假设存在一个理想的匹配函数 $f(.)$，该函数接受两个控件集合并返回成功匹配的控件对集合，即 $f(bf(W)^tar, bf(W)) = {(bf(w)^tar_i, bf(w)_j) | bf(w)^tar_i in bf(W)^tar, bf(w)_j in bf(W) "且匹配成功"}$。当满足以下任一条件时，判定存在不一致性：
...
*流程不一致性（Process Inconsistency）。* 流程不一致性检测本质上是一个考虑拓扑同构的图比较问题。设设计稿上的预期流程为 $bf(p)^tar: G = ( bf(S)^tar, bf(E)^tar )$。由于 $bf(E)^tar$ 可能包含模糊或非原子化的指令，需先进行指令补全与规范化，生成修正后的可执行操作链集 $bf(E)^tar_(c o m p l e t e)$。随后，在实现端执行 $bf(E)^tar_(c o m p l e t e)$ 将在应用程序上生成实际的运行时流程图 $bf(p): G = (bf(S), bf(E)^tar_(c o m p l e t e))$。通过逐一比对设计稿流程图 $bf(p)^tar$ 和实现流程图 $bf(p)$ 之间的对应边，识别并报告满足以下条件的不一致边：
