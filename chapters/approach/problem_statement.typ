...
*屏幕（Screen）。* 我们将屏幕 $bf(s)$ 定义为单个GUI界面，其中包含多个控件，包括图像、按钮、文本等。每个控件 $bf(w)$ 以 $chevron.l x, y, w, h, t chevron.r$ 的格式表示。具体而言，$x, y$ 是控件在屏幕上的左上角坐标，以截图的左上角为原点。$w, h$ 是控件相对于完整截图尺寸的宽度和高度。$t$ 是控件的类型。我们定义了7种主要的控件类型，详见@tbl:widget-class。一个屏幕包含一组控件，即 $bf(W) = {bf(w)_i | bf(w)_i = chevron.l x, y, w, h, t chevron.r}$。

如 @tbl:widget-class 所示，我们将控件分为可交互控件和不可交互控件两类。可交互控件包括文本按钮、图标按钮、组合按钮和输入框。文本按钮通过文本传达其功能，而图标按钮则使用图标。组合按钮同时整合了文本和图标，以提供更全面的表示。输入框允许用户输入文本。不可交互控件包括文本视图、图像视图和图表。
...
*屏幕不一致性（Screen Inconsistency）。* 我们将三种类型的不一致性视为屏幕不一致性，即：多余控件、缺失控件和语义变化。形式化地，我们将设计稿中特定屏幕的控件集合表示为 $bf(W)^tar$，将实现中对应屏幕的控件集合表示为 $bf(W)$。假设存在一个真实匹配函数 $f(.)$，该函数接受两组控件并返回匹配对的集合，即 $f(bf(W)^tar, bf(W)) = {(bf(w)^tar_i, bf(w)_j) | bf(w)^tar_i in bf(W)^tar, bf(w)_j in bf(W) "且二者匹配"}$。当满足以下任一条件时，报告不一致性：

_(i) 实现中缺失控件。_

$ exists bf(w)^tar_i in bf(W)^tar "使得" bf(w)^tar_i product f(bf(W)^tar, bf(W)) $ <eq:miss-widget>
...
$
  exists & (bf(w)^tar_i, bf(w)_j) in f(bf(W)^tar, bf(W)) "使得" \
         & t_i != t_j "或" norm(g(bf(w)^tar_i) - g(bf(w)_j))_2 > epsilon.alt_s
$ <eq:semantic-change>

*流程不一致性（Process Inconsistency）。* 流程不一致性检测从理论上来说是一个考虑拓扑同构的图比较问题。设设计稿上的预期流程表示为 $bf(p)^tar: G = ( bf(S)^tar, bf(E)^tar )$。$bf(E)^tar$ 可能包含模糊的指令，需要自动补全缺失的指令。这将产生修正后的操作链 $bf(E)^tar_(c o m p l e t e)$。在实现上执行 $bf(E)^tar_(c o m p l e t e)$ 会在应用程序上生成一个流程图 $bf(p): G = (bf(S), bf(E)^tar_(c o m p l e t e))$。通过比较设计稿图 $bf(p)^tar$ 和实现图 $bf(p)$ 之间的所有边，我们识别并输出不一致的边：

$
  exists & t, (bf(s)_t^tar, bf(E)_t^tar, bf(s)_(t+1)^tar) in bf(p)^tar, (bf(s)_t, bf(A)_t, bf(s)_(t+1)) in bf(p) \
         & "使得" s i m(bf(s)_(t+1), bf(s)_(t+1)^tar) < epsilon.alt_(s c r e e n)
$ <eq:process-change>
#imagex(
  image(
