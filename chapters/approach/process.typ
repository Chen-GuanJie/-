...
鉴于设计稿的文本描述通常具有歧义且为多模态（文本+图像）形式，VLM 在此承担从模糊描述到标准化动作的桥接作用。例如，对于“trigger the submit button”之类的模糊指令，VLM 会结合带有控件 ID 的当前界面，识别最可能对应的交互控件并返回可执行的原子操作（如 click(widget_1)）。

该过程面临两类主要挑战：界面噪声（大量非目标元素可能干扰匹配）以及 VLM 输出的可解析性（自然语言式回答需被约束为可执行格式）。为此，视觉提示（visual prompt）与控件高亮标注被用于限定 VLM 的关注范围，并将输出限制为预定义的操作集合，从而保证动作可直接被自动化工具解析与回放。

回放成功且判定为与目标界面一致时，若该流程仍包含后续操作，则将当前界面视为新的起始界面继续执行；若不一致且补救无效，则记录为一次流程不一致并在最终报告中列出相应证据与可能的根因。
=== 视觉提示设计 <app:visual-prompt>
为将模糊的设计描述稳定地映射为设备可执行的动作，引入了一个 VLM 模型。输入包括带有控件 ID 注释的当前界面截图与自然语言转移描述，VLM 被要求输出结构化动作序列（例如模板化的 click(widget_id)、send_keys(widget_id, value) 或 JSON 列表），以便解析器能进行确定性解析与后续回放。
...
该VLM模型的目标是在尽可能减少人工干预的同时，确保从多模态设计稿到设备级动作的映射高效、可审计且易于故障追踪。
/*todo
...
界面导航完成后，需要判定跳转结果是否在语义与视觉上与设计稿目标界面相匹配。为此，采用基于控件匹配的定量度量：首先使用@algo:widget-matching 进行控件匹配，并根据@eq:screen-similarity 度量两个界面的相似度。
$
...
其中，$A_(i,j)$ 表示控件 $widget_i$ 与 $widget_j$ 之间的匹配得分见@eq:widget-similarity。
