...
对于设计原型中的每一个状态转移（由源界面 $bf(s)_("src")$、目标界面 $bf(s)_("tar")$ 及其转移描述 $"desc"$ 组成，见 @img:process-eg）
，以及对应的应用中观测到的起始界面 $bf(s)^*("src")$，流程验证包含两个核心环节：

1) 将抽象转移描述映射为可执行动作序列。该映射首先对 $bf(s)_("src")$ 与 $"desc"$ 进行语义解析，生成若干候选动作序列（action candidates）。解析过程中结合自然语言提示、界面上可交互控件的检测结果以及上下文约束，利用视觉-语言模型（VLM）对歧义进行消解并输出格式化的动作序列。为提高可靠性，系统对候选序列进行排序与打分，打分项包括与界面元素的匹配置信度、动作的可执行性估计和历史回放成功率。

2) 在真实设备上回放并验证。选定高置信度的动作序列后，使用自动化执行工具（如 uiautomator @uiautomator2）在应用上逐步回放这些动作，并在每一步采集屏幕截图、UI 层次信息与执行日志。若动作导致界面跳转，则对跳转后的界面与设计原型中的目标界面 $bf(s)_("tar")$ 进行相似度比较（见 @eq:screen-similarity）。相似度不足以判定一致时，将触发补偿策略：
...
- 缺失控件（Missing widget）：列出设计稿中被识别但在实现中未匹配到的控件信息（例如 $bf(w)^{"tar"}_i$），并附带该控件在设计稿中的截图、预期位置与类型说明，以及在实现端未检测到或检测失败的证据（若存在则包含候选相似控件与其相似度评分）。
...
- 流程不一致（Process-level inconsistency）：对未能按预期完成的状态转移，报告实现图中缺失或不匹配的边（例如 $(bf(s)_t, bf(A)_t, bf(s)_{t+1})$），并附带导致失败的步骤序列、回放日志与最终界面差异摘要。

此外，报告将对每一项违规给出：

- 严重度评分（Critical / Major / Minor），基于相似度阈值、功能影响范围与可达性影响评估；
- 优先级与修复建议，例如替换实现中的控件类型、修正文本资源、调整控件位置或修改事件绑定；
- 可追踪的修复项（Issue）格式，支持导出为 JSON 或 HTML 报表，并能与缺陷跟踪系统（如 JIRA）集成以便后续跟进；
