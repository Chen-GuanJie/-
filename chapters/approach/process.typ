...
为将模糊的设计描述稳定地映射为设备可执行的动作，引入了一个受控的 VLM 管道。输入包括带有控件 ID 注释的当前界面截图与自然语言转移描述，VLM 被要求输出结构化动作序列（例如模板化的 click(widget_id)、send_keys(widget_id, value) 或 JSON 列表），以便解析器能进行确定性解析与后续回放。
