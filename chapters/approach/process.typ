...
对于设计原型中的每一个状态转移（由源界面 $bf(screen)_src$、目标界面 $bf(screen)_tar$ 及其转移描述 $"desc"$ 组成，见 @img:process-eg）
...
2) 在真实设备上回放并验证。选定高置信度的动作序列后，使用自动化执行工具（如 uiautomator @uiautomator2）在应用上逐步回放这些动作，并在每一步采集屏幕截图、UI 层次信息与执行日志。若动作导致界面跳转，则对跳转后的界面与设计原型中的目标界面 $bf(screen)_tar$ 进行相似度比较，见@eq:screen-similarity。相似度不足以判定一致时，将触发补偿策略：
...
鉴于设计原型的文本描述通常具有歧义且为多模态（文本+图像）形式，VLM 在此承担从模糊描述到标准化动作的桥接作用。例如，对于“trigger the submit button”之类的模糊指令，VLM 会结合带有组件 ID 的当前界面，识别最可能对应的交互控件并返回可执行的原子操作（如 click(widget_1)）。
...
该管道的目标是在尽可能减少人工干预的同时，确保从多模态原型到设备级动作的映射高效、可审计且易于故障追踪。

=== 界面匹配 <app:screen-matching>

界面导航完成后，需要判定跳转结果是否在语义与视觉上与原型目标界面相匹配。为此，采用基于组件匹配的定量度量：首先使用 @algo:widget-matching 得到更新界面与目标界面之间的匹配对及其相似度权值；随后将所有匹配对的相似度累加并对目标界面的组件数量进行归一化（参见公式 @eq:screen-similarity）。
