...
完成组件匹配后，本工作进一步对匹配结果进行分析与验证，以识别界面实现中与设计原型之间存在的各类不一致性问题，这些问题的定义详见 @sec:ps 章节。

根据组件的匹配状态与特征，系统可识别出三类主要的不一致性现象：

**第一类：多余组件（Extraneous Components）。** 这类组件出现在应用实现中但在设计原型中不存在，或者是匹配过程中未能找到对应的原型组件。多余组件通常表示应用实现中添加了设计未曾指定的功能或UI元素，可能是因为工程实现中出现了额外的业务逻辑或调试信息。

**第二类：缺失组件（Missing Components）。** 这类组件在设计原型中被明确指定但在应用实现中不存在或无法识别，表明实现未能完整遵循设计规范。识别缺失组件有助于发现实现中可能遗漏的功能或交互元素。

**第三类：语义变化（Semantic Variation）。** 这类组件虽然在两个界面版本中都存在并成功匹配，但在类型、内容或视觉表现上产生了变化。

前两类不一致性的识别方法较为直接，即通过将匹配后的组件集合 $bf(W)_1^m$ 和 $bf(W)_2^m$（从设计原型和应用实现中分别获得）
与初始检测到的完整组件集合 $bf(W)_1$ 和 $bf(W)_2$ 进行交集运算与差集分析。未在匹配集合中出现的组件，则分别归类为多余或缺失。

对于第三类语义变化的判断，需在已经成功匹配的组件对上执行更深层的语义检验。判断规则如下：

*类型级不一致：* 若某个匹配对中两个组件的类别标签不相同（例如原型中为 TextButton 而实现中为 TextView），则直接判定为发生了类型级的语义变化，属于严重的设计偏离。

*内容级不一致：* 对于类别相同的匹配对，需根据组件的具体类型进一步检验其内容与视觉表现。本工作参考了 GVT @moran2018automated 方法的做法，对不同类型的组件采用分化的判断策略：

+ 基于文本的组件（TextButton、TextView、InputBox、CombinedButton 等）：需对两个版本中的文本内容进行相似度计算。具体方法为：首先通过 OCR 或从 UI 树中提取文本信息，然后计算基于编辑距离（如莱文斯坦距离）或字符级序列相似度的得分，记为 $"sim"_("text")$。若 $"sim"_("text")$ 超过预设阈值 $epsilon_("ed")$，则认为文本内容基本保持一致；否则视为发生了内容变化。
...
*特殊情形：图表组件（Chart）。* 对于图表类组件，鉴于其内容往往具有高度的动态性与数据驱动特性，设计原型中的图表通常仅作为布局占位符或样式参考，而非精确的数据表示。因此，对图表组件采用更为宽松的比较策略：只要设计原型和实现版本中的图表组件能够成功对齐（即在位置与尺寸上满足相似度阈值），便认为其在设计规范上保持一致，不对数据内容或具体数值进行逐一比对。
