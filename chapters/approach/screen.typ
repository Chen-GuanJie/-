...
== 界面不一致性检测 <app:screen>
在本工作中，首先训练一个计算机视觉模型，用于在设计原型界面和应用实现界面中识别预定义的组件类型。
再将检测到的组件建模为一个可优化的组件对齐问题，并采用动态规划方法进行求解。

=== 组件检测 <app:widget-detection>

本节采用基于视觉的方法识别设计稿和实现界面中的组件。为获得高精度的控件检测能力，首先需构建大规模、高质量的训练数据集。
...
基于上述数据集，训练先进的目标检测模型（如 YOLOv8 @yolov8），可准确识别界面上的各类组件及其在 @sec:ps 节中定义的类型。该方法统一了设计稿与实现两侧的组件提取流程，具备良好的跨应用泛化能力。
...
将组件匹配问题建模为一个动态规划问题，其算法流程如@algo:widget-matching 所示。
算法的输入为来自设计稿和应用实现的两个组件集合，
输出为这两个集合之间的匹配组件对集合。
该算法主要包含以下三个关键步骤：


- *步骤 1：基于递归布局分解的组件偏序关系（@algo:widget-matching 的第 2-- 3 行）。*
传统的基于坐标（$y$-then-$x$）的排序在处理复杂嵌套布局时容易丢失结构语义。为此，本文提出了一种基于递归投影切割（Recursive Projection Cut）的组件线性化算法。
该算法 $cal(L)(widgetset)$ 将无序的组件集合 $widgetset$ 递归地分解为具有视觉逻辑的有序序列。

首先，算法分别沿 $X$ 轴（垂直切分）和 $Y$ 轴（水平切分）计算组件的投影间隙，尝试将集合 $widgetset$ 划分为潜在的子组群 $cal(G)_x$ 和 $cal(G)_y$。
...
为了将组件匹配转化为一个可优化的数值问题，需要对两个组件集合中的每一对候选组件计算精细的相似度得分。该相似度度量采用多模态、分项归一化的策略，以增强对检测噪声及布局差异的鲁棒性。
...
完成组件匹配后，本工作进一步对匹配结果进行分析与验证，以识别界面实现中与设计稿之间存在的各类不一致性问题，这些问题的定义详见 @sec:ps 章节。

根据组件的匹配状态与特征，系统可识别出三类主要的不一致性现象：

*第一类：多余组件（Extraneous Components）。* 这类组件出现在应用实现中但在设计稿中不存在，或者是匹配过程中未能找到对应的设计稿组件。多余组件通常表示应用实现中添加了设计未曾指定的功能或UI元素，可能是因为工程实现中出现了额外的业务逻辑或调试信息。

*第二类：缺失组件（Missing Components）。* 这类组件在设计稿中被明确指定但在应用实现中不存在或无法识别，表明实现未能完整遵循设计规范。识别缺失组件有助于发现实现中可能遗漏的功能或交互元素。

*第三类：语义变化（Semantic Variation）。* 这类组件虽然在两个界面版本中都存在并成功匹配，但在类型、内容或视觉表现上产生了变化。

前两类不一致性的识别方法较为直接，即通过将匹配后的组件集合 $bf(W)_1^m$ 和 $bf(W)_2^m$（从设计稿和应用实现中分别获得）
与初始检测到的完整组件集合 $bf(W)_1$ 和 $bf(W)_2$ 进行交集运算与差集分析。未在匹配集合中出现的组件，则分别归类为多余或缺失。

对于第三类语义变化的判断，需在已经成功匹配的组件对上执行更深层的语义检验。判断规则如下：

*类型级不一致：* 若某个匹配对中两个组件的类别标签不相同（例如设计稿中为 TextButton 而实现中为 TextView），则直接判定为发生了类型级的语义变化，属于严重的设计偏离。
