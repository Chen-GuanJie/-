...
如 @img:meta-model 所示，该元模型旨在形式化描述移动应用的设计结构与交互逻辑，其核心架构由四个关键要素构成：流程（process）、界面（screen）、控件（widget）以及操作（action）。这四个要素共同确立了从宏观业务流转到微观交互细节的完整描述体系。
...
界面是承载可视化信息的容器，而控件则是依附于界面之上的最小交互单元，操作（Action）始终作用于界面中的特定控件之上。本元模型定义了涵盖主流移动端交互行为的操作空间，具体包括：点击（click）、长按（long press）、文本输入（send keys）、滚动（scroll）、滑动（swipe）、拖拽（drag and drop）以及系统级的返回操作（go back）。

当用户在特定控件上触发上述任一操作时，系统状态将发生变迁，结果可归纳为以下三种情形：
（i）*状态内演变*，即操作仅改变当前界面的局部状态而不触发跳转；
（ii）*流程内流转*，即操作导致当前视图切换至同一业务流程内的另一个界面节点；或
（iii）*跨流程跳转*，即操作触发了从当前业务流程向另一个独立业务流程的迁移。

此外，为了确保设计规范的一致性与可验证性，#(tool) 引入了严格的元模型校验机制。如果输入的设计稿违反了上述结构定义或约束条件，系统将自动抛出编译级错误，从工具层面阻断违规输入，并提供详细的错误信息以辅助设计人员对设计稿进行修正与完善。
